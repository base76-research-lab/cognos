"""
exp_011 — Hierarkisk hidden-gate ablation på CIFAR-10.
    ...
    TwoStageHiddenMHC    — backbone → mHC_1 → mHC_2 → mHC_3 → head (+ residual score)
"""
...
class TwoStageHiddenMHC(nn.Module):
    """
    backbone → mHC_1 → mHC_2 → mHC_3 → head

    Hidden-gate och residual-ablation görs i eval-lagret (run.py) via entropi-masker
    och residual_score.
    """
    ...
    def __init__(self, hidden_dim: int = HIDDEN_DIM,
        ...
        self.mhc_1 = mHCLayer(hidden_dim, n_streams)
        self.mhc_2 = mHCLayer(hidden_dim, n_streams)
        self.mhc_3 = mHCLayer(hidden_dim, n_streams)
        self.head = nn.Linear(hidden_dim, n_classes)
    ...
    def forward(self, x: torch.Tensor):
        ...
        r1, H_res_1 = self.mhc_1(h)
        r2, H_res_2 = self.mhc_2(r1)
        r3, H_res_3 = self.mhc_3(r2)
        logits = self.head(r3)

        e1 = routing_entropy(H_res_1)
        e2 = routing_entropy(H_res_2)
        e3 = routing_entropy(H_res_3)
        residual_features = torch.stack([e1, e2, e3], dim=1)
        residual_score = torch.sigmoid(self.residual_head(residual_features)).squeeze(1)

        return logits, H_res_1, H_res_2, H_res_3, residual_score
